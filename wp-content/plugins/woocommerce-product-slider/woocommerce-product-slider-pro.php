...
class Woocommerce_Product_Slider {
		  ...
		  if(empty($first_img)) {
		  	...
		  	if(!empty($options['default_img'])) {
		  		 ...
		  		 $first_img = $options['default_img'];
		   		 ...
		   		 $first_img = plugins_url()."/woocommerce-product-slider-pro/assets/images/default-image.jpg";
	...
	public function get_lazy_load_image($image) {
		  	...
		  	if(!empty($image)) {
		  		 ...
		  		 $lazy_load_image_url = $image;
		   		 ...
		   		 $lazy_load_image_url = plugins_url()."/woocommerce-product-slider-pro/assets/images/loader.gif";
	...
	public function wa_wps_shortcode($atts) {
		...
		#wa_wps_slider_title<?php echo $id;?> { 
		...
		<?php if($wa_wps_image_hover_effect=='hover_image'){ ?>
		...
		#wa_wps_image_carousel<?php echo $id;?> {

			color: <?php echo $options['font_colour']?>;

			font-size: <?php echo $options['font_size']?>px;

			<?php if($wps_pre_direction=="up"||$wps_pre_direction=="down") { ?>

			width: <?php echo $wa_wps_query_posts_item_width; ?>px; 
		...
		#wa_wps_image_carousel<?php echo $id;?> .wa_wps_text_overlay_caption:hover::before {

   			 background-color: <?php echo $options['hover_image_bg']?>!important;
		}
		
		#wa_wps_image_carousel<?php echo $id;?> .wa_wps_prev, #wa_wps_image_carousel<?php echo $id;?> .wa_wps_next,#wa_wps_image_carousel<?php echo $id;?> .wa_wps_prev_v, #wa_wps_image_carousel<?php echo $id;?> .wa_wps_next_v  {
			...
			<?php if($wa_wps_query_animate_controls==1) {  if($wps_pre_direction=="left"||$wps_pre_direction=="right") { ?>
		...
		#wa_wps_image_carousel<?php echo $id;?> .wa_wps_prev:hover, #wa_wps_image_carousel<?php echo $id;?> .wa_wps_next:hover {

			color: <?php echo $options['arrows_hover_colour'];?>;

		}

		#wa_wps_pager_<?php echo $id;?> a {

			background: <?php echo $options['arrows_hover_colour']; ?>;

		}

		#wps_rating_<?php echo $id;?> {

			color: <?php echo $options['arrows_hover_colour']; ?>;

		}

		#wa_wps_image_carousel<?php echo $id;?> li img {
			...
			-webkit-filter: <?php echo $wa_wps_image_hover_effect; ?>(100%); /* Chrome 19+ & Safari 6+ */

			<?php } ?>
		}


		#wa_wps_image_carousel<?php echo $id;?> li .wa_featured_img {

			text-align: <?php echo $options['image_align']; ?>;

		}

		#wa_wps_image_carousel<?php echo $id;?> li  {

			text-align: <?php echo $options['text_align']; ?>;

		}

		#wa_wps_image_carousel<?php echo $id;?> li img:hover {

			<?php if(!empty($wa_wps_image_hover_effect)) { 

			if($wa_wps_image_hover_effect=='border') { ?>

			border : solid 1px <?php echo $options['control_bg_colour']; ?>;

			<?php	} else if($wa_wps_image_hover_effect=='grayscale'||$wa_wps_image_hover_effect=='saturate'||$wa_wps_image_hover_effect=='sepia') { ?>

			filter: none;
			-webkit-filter: <?php echo $wa_wps_image_hover_effect; ?>(0%);
		...
		<script>
		jQuery(document).ready(function() {      	
			jQuery('#wa_wps_foo<?php echo $id; ?>').carouFredSel({<?php if($wps_pre_responsive==1){ echo "responsive:true,"; } ?>
				direction: "<?php echo $wps_pre_direction;?>",
				align: "<?php echo $wps_pre_align;?>",
				<?php if($wps_pre_responsive!=1){ echo "width:'100%',"; } ?>
				auto 	: {
				play:<?php echo $wa_wps_auto;?>,
				timeoutDuration:<?php echo $c_time_out;?>,
				},
				scroll : {
				<?php if(isset($c_items)&&$c_items!=0){ echo 'items:'.$c_items.','; } ?>
				fx: "<?php echo $wa_wps_query_posts_fx;?>",
				easing : "<?php echo $c_easing;?>",
				duration: <?php echo $c_duration;?>,
				<?php if($wa_wps_query_pause_on_hover==1){ ?>					
				pauseOnHover	: true
				<?php } ?>
				},
				infinite: <?php echo $wa_wps_infinite;?>,
				circular: <?php echo $wa_wps_circular;?>,
				<?php if($wa_wps_query_lazy_loading){ ?>
				...
				items: {
					<?php if($wps_pre_responsive==1){ ?> width: <?php echo $wa_wps_query_posts_item_width;?>, <?php } ?>
					<?php if($wps_pre_responsive==0&&$wps_pre_direction=="up"||$wps_pre_direction=="down"){ echo "visible:".$c_min_items.","; }?>   
					<?php if($wps_pre_responsive==1){ ?>                                                 
					visible: {
					min: 1,
				...
				pagination: {
				container: '#wa_wps_pager_<?php echo $id; ?>'
				}
			}
			<?php if($wa_wps_query_css_transitions==1){ ?>
			,{transition:true }
			...
			//touch swipe
			jQuery("#wa_wps_foo<?php echo $id; ?>").swipe({ 
			excludedElements: "button, input, select, textarea, .noSwipe", 
			<?php 	if($wps_pre_direction=="up"||$wps_pre_direction=="down") { ?>swipeUp<?php } else { ?>swipeLeft<?php } ?>: function() { 
			jQuery('#wa_wps_foo<?php echo $id; ?>').trigger('next', 'auto'); 
			}, 
			<?php 	if($wps_pre_direction=="up"||$wps_pre_direction=="down") { ?>swipeDown<?php } else { ?>swipeRight<?php } ?>: function() { 
			jQuery('#wa_wps_foo<?php echo $id; ?>').trigger('prev', 'auto'); 
			...
			container: jQuery("#wa_wps_image_carousel<?php echo $id; ?>")
			});
			}
			<?php } ?>
			//magnific popup
			<?php if($wa_wps_query_posts_lightbox) { ?>
			jQuery('#wa_wps_foo<?php echo $id; ?>').magnificPopup({
			...
			<?php if($wa_wps_query_animate_controls==1) { if($wps_pre_direction=="left"||$wps_pre_direction=="right") { ?>
			jQuery('#wa_wps_image_carousel<?php echo $id; ?>')
		    .hover(function() {
		        jQuery('#wa_wps_image_carousel<?php echo $id; ?> .wa_wps_prev').animate({ 'left' : '1.2%', 'opacity' : 1 }), 300;
		        jQuery('#wa_wps_image_carousel<?php echo $id; ?> .wa_wps_next').animate({ 'right' : '1.2%', 'opacity' : 1 }), 300;
		    }, function() {
		        jQuery('#wa_wps_image_carousel<?php echo $id; ?> .wa_wps_prev').animate({ 'left' : 0, 'opacity' : 0 }), 'fast';
		        jQuery('#wa_wps_image_carousel<?php echo $id; ?> .wa_wps_next').animate({ 'right' : 0, 'opacity' : 0 }), 'fast';
		    });
	    <?php } } ?>
	...
	public function wa_wps_file_path( $view_name, $is_php = true ) {
		...
		$temp_path = get_stylesheet_directory().'/woocommerce-product-slider-pro/themes/';
	...
	public function list_themes() {
		...
		$temp_path = get_template_directory().'/woocommerce-product-slider-pro/themes/';
	...
	public function options_page() {
		...
		foreach($this->tabs as $key => $name) {
			echo '
			<a class="nav-tab '.($tab_key == $key ? 'nav-tab-active' : '').'" href="'.esc_url(admin_url('admin.php?page=woocommerce-product-slider-pro&tab='.$key)).'">'.$name['name'].'</a>';
		}
	...
	public static function default_options() {
		...
		$default_img = plugins_url().'/woocommerce-product-slider-pro/assets/images/default-image.jpg'; // default image
		$loading_img = plugins_url().'/woocommerce-product-slider-pro/assets/images/loader.gif'; // loading image
		$hover_img = plugins_url().'/woocommerce-product-slider-pro/assets/images/hover.png'; // loading image
