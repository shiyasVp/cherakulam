...
add_action( 'after_setup_theme', 'initialize_product_adder_template', 99 );
...
function ic_setup_postdata() {
	if ( is_ic_catalog_page() ) {
		...
		global $post;
		if ( empty( $post->post_content ) && get_integration_type() == 'simple' ) {
			$post->post_content = ' ';
		}
		setup_postdata( $post );
	}
...
function al_product_adder_page_template( $template ) {
	if ( is_ic_catalog_page() ) {
		...
		} else if ( file_exists( get_page_php_path() ) ) {
