...
function ic_is_woo_template_available() {
	if ( !is_integration_mode_selected() && (sample_product_id() !== get_the_ID() || is_archive()) && !isset( $_GET[ 'test_advanced' ] ) ) {
		ob_start();
...
function ic_product_listing_products( $archive_template, $multiple_settings ) {
	...
	if ( (is_tax() || is_search() || !is_ic_only_main_cats() || is_product_filters_active() || is_ic_ajax()) && more_products() ) {
		...
		echo '<div class="product-list responsive ' . $archive_template . ' ' . product_list_class( $archive_template ) . '" ' . product_list_attr() . '>' . $product_list . '</div><span class="clear"></span>';
